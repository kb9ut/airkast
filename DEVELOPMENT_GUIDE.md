...
### 実装済みの主な機能
- **UI:** Jetpack ComposeによるUI。「番組を探す」と「ダウンロード」の2つのタブを持つ。UIは日本語化済み。
- **認証:** アプリ起動後にPC版認証を実行。認証トークンはAirkastClient内部で管理。IPアドレスからエリアを自動判定。
- **番組表:**
    - エリア内の放送局を自動で取得し、選択可能。
    - 過去7日間の日付をドロップダウンで選択可能。
    - 選択された放送局・日付の番組表を表示。
- **ダウンロード (バックグラウンド対応):**
...
- **再生 (バックグラウンド対応):**
    ...
    - 「再生」ボタンで`PlayerService` (MediaLibraryService) を起動し、アプリを閉じても再生が継続。
    - OSのメディアコントロール（通知、ロック画面など）から再生制御が可能。
    - 倍速再生対応。
    - **再生位置の自動保存・復元:** アプリを閉じても、次回再生時に続きから再生可能。
- **削除:**
    - 「削除」ボタンでファイルを削除。
- **その他:**
    - 番組の放送時間は「14:00 - 15:30」のような読みやすい形式で表示。
...
3. 内部的に `HlsDownloader` を利用してHLSストリーム (.m3u8) を解析し、AACセグメントファイルをダウンロード・結合します。
4. ダウンロードの進捗は、アプリの通知としてユーザーに表示され、また「ダウンロード」タブのUIにもリアルタイムで反映されます。
